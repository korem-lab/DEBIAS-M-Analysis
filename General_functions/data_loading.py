...
def load_CRC(data_path):
    df = pd.read_csv('../data/CRC/data_relabund.csv', index_col=0)#.T
    md = pd.read_csv('../data/CRC/metadata_relabund.csv', 
                     index_col=0
...
def load_Metabolites(data_path):
    md = pd.read_excel('../data/Metabolites/Supplementary Tables.xlsx', 
             sheet_name='Table S1', 
             engine='openpyxl', 
                  index_col=0)
    
    mds = []
    for fp in glob.glob('../data/Metabolites/new/HSRR*.txt'):
    ...
    df = pd.read_csv('../data/Metabolites/new/otu.csv', 
                     index_col=0 ).loc[md.index]

    df = df.loc[:, ( df > 0 ).sum(axis=0)>(.05*df.shape[0]) ]

    tbl = pd.read_excel('../data/Metabolites/new/41467_2019_9285_MOESM5_ESM.xlsx', 
