...
class PL_DEBIASM_SW(pl.LightningModule):
    ...
    def training_step(self, batch: Tuple[Tensor, Tensor], batch_idx: int) -> Dict[str, Tensor]:
        ...
        if self.batch_sim_str > 0:
            ...
            loss += sum( [ torch.sum(self.bs==self.unique_bs[i])*\
                           pairwise_distance(x1, a) \
                           for i,a in enumerate(x1)] ).sum() *\
                                    self.batch_sim_str / self.unique_bs.shape[0] ##to scale by ~~the batch sizes
...
def DEBIASM_SW_train_and_pred(X_train, 
                              X_val, 
                              y_train, 
                              y_val,
    ...
    model = PL_DEBIASM_SW( X = torch.tensor( np.vstack((X_train, X_val)) ),
...
class DebiasMSWClassifier(BaseEstimator):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        preds, mod = DEBIASM_SW_train_and_pred(
                                               X, 
                                               self.x_val, 
                                               y, 
                                               0,
