...
def main(out_path='res-crc-n-per-study.csv'):
    ...
    for __ in range(n_reps):
        ...
        for n_per_study in [20, 40, 60, 80, 100]:
            i+=1
            all_rocs=[]
            all_names=[]
            n_per_study_list=[]
            ...
            out=( n_per_study / md_tmp.Study.value_counts() ).reset_index()\
                            .apply(lambda x: md_tmp.loc[md_tmp.Study==x['Study']].groupby('label').sample(frac=min(1, 
                                                                                                                   x['count']
                                                                                                                   )), 
                                   axis=1)
            md_tmp = pd.concat( [a for a in out] )
            print(md_tmp.shape)
            print(md_tmp['Study'].value_counts())
            ...
            df_tmp=pd.DataFrame(1e-6+rescale(df.loc[md_tmp.index].values ), ## the 1+ avoids some nans in the baseline
                                index=md_tmp.index )
...
#             ### run the R script
            ...
            for a in pths:
                ...
                n_features_per_sample += [ get_nunique_feature_summary(tt) ]*len(sc)
            ...
            for train_inds, test_inds in\
                ...
                if ( md_tmp.label.loc[test_inds].nunique()>1 )\
                    ...
                    n_per_study_list.append(n_per_study)
                    all_names.append('Linear')
                    n_features_per_sample.append( get_nunique_feature_summary(df_tmp.values ) )
                    ...
                    n_features_per_sample.append( get_nunique_feature_summary( dmc.transform( df_tmp_with_batch.values ) ) )


            results_df_n_per_study = \
                    pd.DataFrame({'auROC':all_rocs, 
                                  'Group':all_names,
                                  'N per study':n_per_study,
                                  'N features per sample':n_features_per_sample,
                                  'Run num':i
                                 })
