...
class PL_SBCMP_regression(pl.LightningModule):
    ...
    def training_step(self, batch: Tuple[Tensor, Tensor], batch_idx: int) -> Dict[str, Tensor]:
        ...
        # PyTorch mse loss
        loss = F.mse_loss(y_hat.squeeze(-1), y, reduction="mean") * self.y_loss_scaling
    ...
    def validation_step(self, batch: Tuple[Tensor, Tensor], batch_idx: int) -> Dict[str, Tensor]:
        ...
        loss = F.mse_loss(y_hat.squeeze(-1), y, reduction="sum") * self.y_loss_scaling
...
class DebiasMRegressor(BaseEstimator):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if self.mse_scaling=='infer':
            self.mse_scaling = 1e3/np.var(y) ## assuming unit standard scaling in general, so=>1e3
