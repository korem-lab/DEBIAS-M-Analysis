...
class PL_DEBIASM_unib(pl.LightningModule):
    ...
    def training_step(self, batch: Tuple[Tensor, Tensor], batch_idx: int) -> Dict[str, Tensor]:
        ...
        if self.batch_sim_str > 0:
            x1 = F.normalize( torch.pow(2, self.batch_weights ) * x, p=1 )
            loss += self.batch_sim_str * torch.std( x1, dim=0).sum()
