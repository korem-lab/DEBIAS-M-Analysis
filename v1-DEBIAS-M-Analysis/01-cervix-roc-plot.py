...
def main():
    ...
    for a in results_dict['models']:
        ...
        if a == 'debias-m':
            ...
            preds = results_dict['models'][a][0][1].predict_proba( 
                results_dict['models'][a][0][0].transform( rescale(percnorm.loc[inds] ) ) )[:, 1]
            nm='Percentile normalization'
            
        elif a == 'mmuphin':
            preds = results_dict['models'][a][0][1].predict_proba( 
                results_dict['models'][a][0][0].transform( rescale(mmup.loc[inds]) ) )[:, 1]
            nm='MMUPHin'
            
            
        elif a == 'plsda':
            preds = results_dict['models'][a][0][1].predict_proba( 
                results_dict['models'][a][0][0].transform( plsda.loc[inds]) )[:, 1]
            nm='PLSDAbatch'
    ...
    pd.concat( aupr_dfs ).to_csv('../results/Cervix-carcinoma/linear-benchmark-prs.csv')        
    ...
    for a in results_dict['models']:
        ...
        if a != 'snm':
            ...
            if a == 'debias-m':
                ...
                preds = results_dict['models'][a][0][1].predict_proba( 
                                results_dict['models'][a][0][0].transform( results_dict['clr_datasets']['percnorm'].loc[inds]) )[:, 1]
                nm='Percentile normalization'
                
            elif a == 'mmuphin':
                preds = results_dict['models'][a][0][1].predict_proba( 
                                results_dict['models'][a][0][0].transform( results_dict['clr_datasets']['mmuphin'].loc[inds]) )[:, 1]
                nm='MMUPHin'
