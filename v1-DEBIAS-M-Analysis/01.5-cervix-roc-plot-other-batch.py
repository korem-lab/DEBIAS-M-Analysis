...
def main():
    ...
    for task in task_mapping_dict:
        ...
        mmup=pd.read_csv('../data/Cervix/MMUPHin_out.csv', index_col=0)
        plsda=pd.read_csv('../data/Cervix/PLSDAbatch_out.csv', index_col=0)
        percnorm=pd.read_csv('../data/Cervix/percnorm_out.csv', index_col=0)
        ...
        mmup=mmup.loc[df.index]
        plsda=plsda.loc[df.index]
        percnorm=percnorm.loc[df.index]
        ...
        results_dict = all_classification_pipelines.run_predictions(y.loc[inds], 
                                                                    ...
                                                                    df_mup=mmup.loc[inds], 
                                                                    df_pls=plsda.loc[inds],
                                                                    df_perc=percnorm.loc[inds]
                                                                   )
    ...
    for a in results_dict['models']:
        ...
        if a == 'debias-m':
            preds = results_dict['models'][a][1].forward(torch.tensor(df_with_batch.loc[inds].values ).float() )[:, 1]                    .detach().numpy()
            nm = 'DEBIAS-M'
            pd.concat([ md.loc[inds].label, 
             ...
             ).to_csv('../results/Cervix-carcinoma/other-study-carcinoma-debiasm-linear-predictions.csv')
            
            
        elif a == 'linear':
            preds = results_dict['models'][a][1][1].predict_proba( 
                        results_dict['models'][a][1][0].transform(
            ...
            pd.concat([ md.loc[inds].label,
                 ...
                 ).to_csv('../results/Cervix-carcinoma/other-study-carcinoma-raw-linear-predictions.csv')

        elif a == 'conqur':
            preds = results_dict['models'][a][1][1].predict_proba( 
                results_dict['models'][a][1][0].transform( rescale( conq_out.loc[inds]) ) )[:, 1]
            nm='ConQuR'

        elif a == 'combat':
            preds = results_dict['models'][a][1][1].predict_proba( 
                            results_dict['models'][a][1][0].transform( rescale( comb_out.loc[inds]) ) )[:, 1]
            nm='ComBat'

        elif a == 'snm':
            preds = results_dict['models'][a][1][1].predict_proba( 
                results_dict['models'][a][1][0].transform( snm_out.loc[inds]) )[:, 1]
            nm='Voom-SNM'
            
        elif a == 'percnorm':
            preds = results_dict['models'][a][1][1].predict_proba( 
                results_dict['models'][a][1][0].transform( rescale(percnorm.loc[inds] ) ) )[:, 1]
            nm='Percentile normalization'
            
        elif a == 'mmuphin':
            preds = results_dict['models'][a][1][1].predict_proba( 
                results_dict['models'][a][1][0].transform( rescale(mmup.loc[inds]) ) )[:, 1]
            nm='MMUPHin'
            
            
        elif a == 'plsda':
            preds = results_dict['models'][a][1][1].predict_proba( 
                results_dict['models'][a][1][0].transform( plsda.loc[inds]) )[:, 1]
            nm='PLSDAbatch'
        ...
        pre,rec,_= precision_recall_curve(md.loc[df_with_batch[0]==bi].label, 
                             preds)
        ...
        name_pr = nm + ' (auPR = {:.2f})'.format(
                    average_precision_score(md.loc[df_with_batch[0]==bi].label, 
                             preds))
        global_palette[name_pr] = global_palette[a]
        ...
        aupr_dfs.append( pd.DataFrame({'Precision':pre, 
                                       'Recall':rec, 
                                       'Group':[name_pr]*pre.shape[0]}))

    combined = pd.concat(roc_dfs)
    combined.to_csv('../results/Cervix-carcinoma/other-study-linear-benchmark-rocs.csv')
                                       
    pd.concat( aupr_dfs ).to_csv('../results/Cervix-carcinoma/other-study-linear-benchmark-prs.csv')        
    ...
    plt.savefig('../results/Cervix-carcinoma/other-study-cervix-carcinoma-roc.pdf', 
                bbox_inches='tight', 
                dpi=900, 
                format='pdf', 
               )
    ...
    for a in results_dict['models']:
        ...
        if a != 'snm':
            ...
            inds = md.loc[df_with_batch[0]==bi].index
            if a == 'debias-m':
                preds = results_dict['models'][a][1]                    .forward(torch.tensor(results_dict['clr_datasets']['raw'].loc[inds].values ).float() )[:, 1]                        .detach().numpy()
                nm = 'DEBIAS-M'
            elif a == 'linear':
                preds = results_dict['models'][a][1][1].predict_proba( 
                            results_dict['models'][a][1][0].transform(
                ...
                preds = results_dict['models'][a][1][1].predict_proba( 
                    results_dict['models'][a][1][0].transform( results_dict['clr_datasets']['conqur'].loc[inds]) )[:, 1]
                nm='ConQuR'

            elif a == 'combat':
                preds = results_dict['models'][a][1][1].predict_proba( 
                                results_dict['models'][a][1][0].transform( results_dict['clr_datasets']['combat'].loc[inds]) )[:, 1]
                nm='ComBat'
                
            elif a == 'percnorm':
                preds = results_dict['models'][a][1][1].predict_proba( 
                                results_dict['models'][a][1][0].transform( results_dict['clr_datasets']['percnorm'].loc[inds]) )[:, 1]
                nm='Percentile normalization'
                
            elif a == 'mmuphin':
                preds = results_dict['models'][a][1][1].predict_proba( 
                                results_dict['models'][a][1][0].transform( results_dict['clr_datasets']['mmuphin'].loc[inds]) )[:, 1]
                nm='MMUPHin'
    ...
    plt.savefig('../results/Cervix-carcinoma/other-study-cervix-clr-carcinoma-roc.pdf', 
                bbox_inches='tight', 
                dpi=900, 
                format='pdf', 
               )
